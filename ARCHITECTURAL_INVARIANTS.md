...
| 4 | **OpenAPI codegen freshness** — frontend `types/api.d.ts` matches backend OpenAPI spec. | Backend type changes silently break Vue forms. | Claimed gate `tools/openapi_drift_gate.sh` does not exist in the repo and CI has no openapi step (verified 2026-08-18); `api.d.ts` regenerated manually on the email-overhaul branch. Per policy rule 4, this row was misfiled. | **documented-only** |
