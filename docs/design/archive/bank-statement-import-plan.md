...
**Attestations, not exclusive ownership.** The real corpus contains the triggering case (§11 finding 5): a line first seen in the May savings import is re-attested by the overlapping June restatement. With only `import_id`, voiding May would silently delete evidence June still vouches for. Every import — first-seen or deduped — writes an attestation row; **voiding an import deletes its attestations and then only lines left with zero attestations**. If a **confirmed** match references a line the void would delete, the void itself is **refused** (409 — unconfirm first): stronger than the originally-sketched exception queue, and never a silent unlink; suggested/rejected matches are derived data and drop with their lines. Lines are never edited; a bad import is always voided by batch.
...
3. **PR 3 — verification (BUILT):** migration 052 (the §4 three-table shape with partial-unique exclusivity on denormalized child status), the matcher (R5 classification incl. both transfer grammars → R2 exact-1:1 with bipartite-degree-1 refusal → R3 unique-subset deposit sweep, ambiguity always refused → manual), suggest/confirm/reject/unconfirm/manual endpoints (`accounting.write`), the Reconcile tab, and the four reports (unmatched deposits / payments never seen by the bank / date drift / unmatched debits). Suggest-only throughout — nothing auto-confirms. First live run against the real corpus + real dev-DB payments: 34 suggestions, confirm flow verified, reports quantified the actual verification gap. R1/R4/R6 from the Phase 2 ladder are deliberately absent in v1: R1's check-number↔`Payment.reference` anchor doesn't exist in this bank's statement text (deposits are batched slips; per-check numbers appear only in the scans), R4 tolerance and R6 aging land with real usage.
4. **Later (separate effort) — ⚠ PARTLY NOW DUE, and one part is a live
   defect.** The "re-target to GL lines" deferral is why nothing credits 1050:
   $80,291.99 of cash/check payments are stranded there on prod as of
   2026-08-22, because this matcher pairs statement lines to **Payment rows**
   and never posts the clearing entry. Now scoped in
   `undeposited-funds-clearing-plan.md`. Original text:
   GL Phase 2 proper — re-target matcher right-hand side to GL lines, tie-out-to-GL assertion, period locks, posting rules. Evidence tables unchanged. Banno feeder converges here too (its `line_hash` gains `occurrence_n` at that point, per the Phase 2 note).
...
A **sixth, integration audit ran the stack against the EXISTING system** (payment lifecycle, QB/Stripe payment creators, role bundles, module system, Banno coexistence, single-tenant conventions; 274 coupled existing tests re-run — zero regressions). Its headline, proven by probe: **the books could mutate under confirmed matches invisibly** — the payment-void endpoint (and expense soft-delete, vendor-bill void, GDPR hard-deletes) knows nothing about matches, so a payment voided after confirmation left its deposit "settled by dead money" with every report silent, while the invoice said unpaid. Fixed: `build_reports` now sweeps confirmed externals for dead records (voided / deleted / void-status / row missing) into a loud **broken_matches** section with an Unconfirm action in the UI — the match is never silently unlinked, but the contradiction can no longer hide; dead payments also stop emitting date-drift rows. Second find, also proven: the processor-method exclusion was **guesses, not the codebase's vocabulary** — QB-imported payments write `method="quickbooks"` (instrument unknown, includes hidden card money) and Stripe ACH writes `"ach"`; a probe completed a $500 sweep from exactly that pair, and R2 had no method filter at all (a card payment 1:1-matched a deposit). Fixed: `card`-family + `ach` excluded from both R2 and R3; `quickbooks` allowed exact R2 (amount+date is strong evidence regardless of instrument) but excluded from R3 sweeps; all remain visible in the manual dialog with method shown. Both decision items **RESOLVED by Doug (2026-08-01)**: statement import/void is the accounting role's job → new `bank_feeds.statements` permission (its own key — widening `bank_feeds.manage` would also have granted Banno credential management), in the code bundle AND backfilled into existing `tenant_roles` accounting rows by **migration 053** (the resolver honors the DB row, not the code bundle — the S97 perm-snapshot trap); and check/deposit-ticket scans are now **higher-office only** → the scan file endpoint requires `accounting.write` (accounting/admin/owner yes; viewer/tech/dispatcher no), with the UI hiding the gallery and showing "restricted to accounting staff" for everyone else. Also noted: if Banno ever connects, the same money will exist in both `bank_feed_transactions` and `bank_statement_lines` with no cross-reference until the GL Phase 2 unification — nothing reads both today.

A **fifth, whole-stack audit ran across all three PRs as a unit** — hunting the seams the per-diff audits structurally couldn't see — and found the feature's worst bug: **manual matches committed with children stuck at `suggested` under a `confirmed` parent** (the repo-wide `autoflush=False` session config meant `_sync_children`'s SELECT couldn't see the pending child rows). Consequences it proved by execution: the four reports contradicted each other about the same match (settled in date-drift, unsettled everywhere else), and the void seam — which trusts child status — silently deleted confirmed manual matches, defeating its own protection. Fixed with a flush before sync + a regression test asserting committed child state. It also forced: **sticky rejection** (a rejected pairing's exact (line-set, external-set) signature is never re-suggested; different pairings for the same line still are), the reject endpoint now requiring unconfirm-first on confirmed matches (it was a one-click destroyer while void had a ceremony), a bounded upload read (the 10 MB cap ran after a full read into memory), and this section's own §4 wording corrected (the "exception state" it promised never existed — refuse-the-void is the real, stronger behavior). Verified coherent under attack: money signs across all five layers, both deploy sequences live on scratch Postgres, endpoint gating on all 16 routes, and the void × attestation × match lifecycle matrix. Earlier the same day, my own pre-stack-audit check caught the void↔matches FK seam (commit 88f9c99). 48 tests now cover the statement suites.

A **fourth adversarial audit ran on the PR 3 diff** (matcher + reports) and reproduced three defects with executed tests before verdicting "do not commit": the vendor-invoice rung had no consumption tracking — two same-amount debits near one bill emitted the same external twice and the **entire suggest run died on the exclusivity index at commit** (fixed: full bipartite degree-1 discipline on both debit rungs, same as deposits); the suggestion **wipe could delete a just-confirmed match** in a suggest-vs-confirm race (fixed: per-account Postgres advisory lock, status-re-checked row-locked deletes, and commit-time IntegrityError → 409 "nothing was saved, retry"); and R2's "no competing candidate" was **only checked inside the requested window/account** — a same-amount deposit days outside the range, or on the other account, was an invisible rival (fixed: the competitor map is built over a padded, all-accounts window). Also forced: the expense rung's silent first-come assignment became degree-1 refusal; the Suggestions table now honors the date pickers; five missing refusal tests added (43 total). Judgment call it surfaced, kept deliberately: report settlement for payments is **bank-wide, not per-account** (a payment deposited into savings is verified reality; per-account scoping would falsely re-flag it) — declared in the payload as `payments_scope: "bank_wide"`.
