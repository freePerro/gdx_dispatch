...
**Date:** 2026-07-17 · **Status:** RELEASED v1.101.0 (#481) — verified against `origin/main @ bb25a48`, 2026-08-25. Every item this plan scopes is built: PR A (`routers/mobile.py:980 _job_card`, the one job shape both paths use), PR B (the detail view carries the actions, parts, closeout and photo capture — `MobileJobDetailView.vue` `usePhotoQueue`), and the **"BOTH clocks"** section (`routers/mobile.py:733,2610`; `MobileJobDetailView.vue:668,703,714,1489,1520`). §"Explicitly NOT in this PR" defers card payment and the day-vs-job variance report out of scope — the latter is now revisitable, since per-job time is finally captured.

> **Correction, 2026-08-25.** This line previously read "Still not built: reorder mode with the new card, drive-time legs between stops, and the map view." **All three were already built, and none of them are this plan's scope** — they belong to `mobile-one-job-card-plan.md`. They live in `frontend/src/views/MobileTodayView.vue` (the `VIEW_MAP` branch, the `leg-eta` drive legs off `drive_time_to_next_seconds`, and the reorder controls rendered beside `MobileJobCard`). The error came from carrying the previous status line forward instead of re-deriving it from code — the exact failure this plan's own corpus audit exists to catch. Docs state; code proves.
