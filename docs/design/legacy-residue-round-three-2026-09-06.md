...
**Status:** `PARTIALLY BUILT` — **PR A (prose, docstrings, comments, the dead S3 backup script) is this PR.** B, C and D not started. Successor to
...
| B10 | `docker/docker-compose.staging.yml` | 43 lines: a `control-db` service and `gdx_staging_control` database; referenced by nothing | Delete |
