...
**Date:** 2026-09-18
**Status:** RELEASED v1.122.0 (2026-09-21; prod and demo updated, plugin-host on both now receives `APP_VERSION`). Core keeps only the webhook shim (`routers/cell_gateway.py`, its compose variable, its tests) — #752 merged 2026-09-20, the in-tree package removed by #763. The plugin lives in `gdx_dispatch_plugins` (PR #5) and is listed in the storefront from that repo's tag v0.2.0 (2026-09-21) with `requires = "gdx>=1.122.0"`. Not yet installed on prod: the owner's install and the owed-before-prod-use items below are still open. Store-all ingest + XML backfill in one build per Doug's ruling. Owed before prod use: AVD nomad-payload verification, browser walk, the three deploy checks in the plugin README, a local stack for the browser walk with `APP_VERSION=1.122.0` set (an unpinned `dev`/`latest` host reads as version 0 and skips a pinned plugin), and a ruling on the shim's 404 pass-through — with the secret set and the plugin not yet installed, every text/call is answered 404 with no log line and no audit row (found by the 2026-09-20 audit; the shim is unchanged, the case is pinned in `tests/test_cell_gateway.py`).
