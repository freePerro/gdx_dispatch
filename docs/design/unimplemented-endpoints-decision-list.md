...
| 11 | `PATCH /api/jobs/{id}/parts/{part_id}` | `JobCostingView` | `JobPart` exists; only POST was built. Note `GET`/`DELETE` on the same resource are also broken (C2). | **Build the full CRUD** — the parts panel is non-functional without it. | ⚠️ **DECIDED, DESIGNED, BUILT — AND PULLED BEFORE MERGE.** Owner 2026-08-25: *"yes parts cost should be tracked"*, and the design: *"the cost should be close to what we use out of the catalog for estimating, until an invoice line is pointed at it."* The investigation stands and is worth keeping: `job_parts` is empty because it **FKs to the `parts` catalog, which is also empty**, so a cost row is structurally impossible; the estimator actually prices from `custom_catalog_items` (2,858 rows / 2,664 costed) and `chi_parts_catalog`; and `vendor_invoice_lines` has the `job_id`/`job_part_needed_id` columns, index and FK for real cost with **0 rows attributed**. 🔴 The three-tier implementation was **pulled before merge** by an adversarial review: `confirm.py` does not link a bill line to the tech's original request row — it MINTS A NEW `JobPartNeeded` and links to that — so a dedup keyed on `job_part_needed_id` leaves the original row still taking a catalog estimate **on top of** the actual bill. Every attributed part would have counted twice. Root cause: `job_parts_needed` is an **event log** (one row per request/closeout/mobile/van/vendor_invoice event, duplicates by design), so summing it is a category error. The rework must first decide which rows *count* — by `source`, `status`, `billed_invoice_id` — before anything prices them. Full findings, including freight/tax counted as parts, soft-deleted bills still charging, a UUID form mismatch that makes tier 1 silently never fire on SQLite, and an N+1 performance problem: **#469**. |
