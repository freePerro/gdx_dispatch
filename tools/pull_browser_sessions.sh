#!/usr/bin/env bash
# Pull plugin browser-stream session recordings from the VPS to this machine.
#
...
#
#   ./pull_browser_sessions.sh              # sync new/changed recordings
#   ./pull_browser_sessions.sh --verify     # sync, then check every session's
#                                        # manifest count against the local file
#   DEST=/somewhere ./pull_browser_sessions.sh
#
...
DEST="${DEST:-$HOME/Desktop/browser-sessions}"
STAGE="/tmp/browser-sessions-stage"
