...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    msg = (
        ...
        "\n"
        "    There is usually NO host .venv — deps live in the docker-app\n"
        "    image. Drive the split runner with the image via $PYTEST:\n"
        "            PYTEST=\"docker run --rm --entrypoint python \\\n"
        "              -e JWT_SECRET=<32+ bytes> -v $PWD:/app -w /app \\\n"
        "              docker-app -m pytest\" \\\n"
        ...
        "              docker-app -m pytest gdx_dispatch/tests/test_<feature>.py -v\n"
        "\n"
        "    Override (rare — debugging cross-test pollution):\n"
        "            PYTEST_FULL_SERIAL=1 <pytest as above> gdx_dispatch/tests/\n"
        "\n"
