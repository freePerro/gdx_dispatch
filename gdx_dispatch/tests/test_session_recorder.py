...
def _rec(tmp_path, **kw):
    return SessionRecorder(
        ...
        url="https://orderentry.chiohd.com/", root=str(tmp_path), **kw
    )
...
async def test_records_a_session_end_to_end(tmp_path):
    rec = _rec(tmp_path)
    await rec.start()
    rec.note_client(json.dumps({"type": "nav", "url": "https://orderentry.chiohd.com/cart"}))
...
async def test_saved_session_keeps_cookie_names_but_never_values(tmp_path):
    ...
    rec.note_server(json.dumps({
        "type": "session",
        "state": {"cookies": [
            {"domain": "orderentry.chiohd.com", "name": "CHI.OrderEntry.Web.Prod",
    ...
    assert "orderentry.chiohd.com|CHI.OrderEntry.Web.Prod" in ev["cookie_names"]
...
async def test_credential_bearing_urls_are_scrubbed(tmp_path):
    ...
    dirty = "https://orderentry.chiohd.com/signin-oidc?code=SECRETAUTHCODE&state=abc"
    assert "SECRETAUTHCODE" not in scrub_url(dirty)
    assert "orderentry.chiohd.com/signin-oidc" in scrub_url(dirty)
    ...
    # A normal HubX URL must survive intact — over-scrubbing loses the trail.
    plain = "https://orderentry.chiohd.com/cart/Quote/3/abc/door/view/def/GARA01"
