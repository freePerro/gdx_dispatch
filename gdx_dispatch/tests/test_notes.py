...
def test_create_sticky_note(client: TestClient):
    r = client.post(
        "/api/sticky-notes",
        json={
            "title": "Reminder",
            "body": "Call the parts supplier",
            "color": "#fef3c7",
            "pos_x": 100,
            "pos_y": 200,
        },
    ...
    assert data["body"] == "Call the parts supplier"
    assert data["pos_x"] == 100
    assert data["pos_y"] == 200
    assert data["color"] == "#fef3c7"
    assert data["width"] == 240
    assert data["height"] == 180
    assert data["company_id"] == "tenant-test"

    listed = client.get("/api/sticky-notes").json()
    assert any(n["id"] == data["id"] for n in listed)


def test_sticky_note_move(client: TestClient):
    created = client.post(
        "/api/sticky-notes",
        json={"body": "Movable", "pos_x": 0, "pos_y": 0},
    ).json()
    r = client.post(
        f"/api/sticky-notes/{created['id']}/move",
        json={"pos_x": 500, "pos_y": -250},
    )
    assert r.status_code == 200, r.text
    data = r.json()
    assert data["pos_x"] == 500
    assert data["pos_y"] == -250

    # out-of-bounds move rejected
    bad = client.post(
        f"/api/sticky-notes/{created['id']}/move",
        json={"pos_x": 999999, "pos_y": 0},
    )
    assert bad.status_code == 422


def test_soft_delete_sticky(client: TestClient):
    created = client.post(
        "/api/sticky-notes",
        json={"body": "To be deleted"},
    ).json()
    r = client.delete(f"/api/sticky-notes/{created['id']}")
    assert r.status_code == 204

    listed = client.get("/api/sticky-notes").json()
    assert all(n["id"] != created["id"] for n in listed)

    # Follow-up operations on a soft-deleted sticky return 404
    move = client.post(
        f"/api/sticky-notes/{created['id']}/move",
