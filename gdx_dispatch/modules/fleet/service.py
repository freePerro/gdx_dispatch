...
def log_service(vehicle_id: UUID, service_type: str, mileage: int, service_date: datetime, cost: float | None, notes: str | None, db: Session, actor: str = SYSTEM_ACTOR) -> VehicleServiceRecord:
    ...
    db.add(row); db.flush(); asyncio.run(log_audit_event(db, "vehicle_service_logged", actor, "vehicle", str(vehicle_id), {"service_type": service_type, "mileage": mileage})); db.commit(); db.refresh(row)  # noqa: E701,E702
    return row
...
def update_odometer(vehicle_id: UUID, new_odometer: int, db: Session, actor: str = SYSTEM_ACTOR) -> Vehicle:
    vehicle = db.execute(select(Vehicle).where(Vehicle.id == vehicle_id, Vehicle.deleted_at.is_(None))).scalar_one_or_none()
    if not vehicle:
        raise HTTPException(status_code=404, detail="Vehicle not found")
    vehicle.odometer = new_odometer
    asyncio.run(log_audit_event(db, "odometer_updated", actor, "vehicle", str(vehicle_id), {"odometer": new_odometer})); db.commit(); db.refresh(vehicle)  # noqa: E701,E702
    return vehicle
