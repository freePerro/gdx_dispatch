...
def enroll_customer(customer_id: UUID, plan_id: UUID, stripe_sub_id: str | None, db: Session, actor: str = SYSTEM_ACTOR) -> CustomerPlanEnrollment:
    ...
    db.add(row); db.flush(); asyncio.run(log_audit_event(db, "plan_enrolled", actor, "customer_plan_enrollment", str(row.id), {"customer_id": str(customer_id), "plan_id": str(plan_id)})); db.commit(); db.refresh(row)  # noqa: E701,E702
    return row


def schedule_next_visit(enrollment_id: UUID, db: Session, actor: str = SYSTEM_ACTOR) -> CustomerPlanEnrollment:
    ...
    asyncio.run(log_audit_event(db, "next_visit_scheduled", actor, "customer_plan_enrollment", str(row.id), {"next_service_at": row.next_service_at.isoformat()})); db.commit(); db.refresh(row)  # noqa: E701,E702
    return row


def cancel_plan(enrollment_id: UUID, db: Session, actor: str = SYSTEM_ACTOR) -> CustomerPlanEnrollment:
    row = db.execute(select(CustomerPlanEnrollment).where(CustomerPlanEnrollment.id == enrollment_id)).scalar_one_or_none()
    if not row:
        raise ValueError("Enrollment not found")
    # Stripe cancellation is handled by the caller (for example, a Celery task).
    row.status, row.canceled_at = "canceled", utcnow()
    asyncio.run(log_audit_event(db, "plan_canceled", actor, "customer_plan_enrollment", str(row.id), {"stripe_subscription_id": row.stripe_subscription_id}))
