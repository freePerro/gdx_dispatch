...
class Principal:
    ...
    def from_pat(
        ...
        *,
        ...
        """Construct a PAT-flow principal (SS-14).

        ``pat_row`` is the SQLAlchemy ``AccessToken`` instance matched by
        :func:`gdx_dispatch.core.pat_validation.validate_pat`. ``pat_row.id`` and
        ``pat_row.prefix`` are read off the row — we don't re-fetch.
        """
        pat_id_raw = getattr(pat_row, "id", None)
        if pat_id_raw is None:
            raise ValueError("pat_row.id is required")
        pat_id = pat_id_raw if isinstance(pat_id_raw, UUID) else UUID(str(pat_id_raw))
        pat_prefix = getattr(pat_row, "prefix", None)
        if not isinstance(pat_prefix, str) or not pat_prefix:
            raise ValueError("pat_row.prefix must be a non-empty str")
        caps_t = _coerce_caps(capabilities)
        return cls(
            identity_id=identity_id,
            tenant_id=tenant_id,
    ...
    def from_scim(
        ...
        *,
        ...
        tenant_id: str,
        capabilities: Iterable[tuple[str, str]],
        role: str = "scim_client",
        is_restricted: bool = False,
    ) -> Principal:
        """Construct a SCIM-flow principal (SS-22).

        ``token_record`` is the SCIM token row/record whose ``token_id``
        (or ``id`` — we accept either) survives onto the principal for
        audit trails.
        """
        token_id = getattr(token_record, "token_id", None) or getattr(
            token_record, "id", None
        )
        if token_id is None and isinstance(token_record, dict):
            token_id = token_record.get("token_id") or token_record.get("id")
        if token_id is None:
            raise ValueError("token_record must expose token_id or id")
        caps_t = _coerce_caps(capabilities)
        return cls(
            identity_id=identity_id,
            tenant_id=tenant_id,
