"""Full-detail recording of plugin browser-stream sessions (e.g. CHI HubX quoting).

Why this exists
---------------
CHI HubX has no usable API — its UI is a Blazor Server projection pushed over a
SignalR circuit, and its REST surface answers 403 (origin gate) then 401. The
only way we can ever learn how CHI prices a door is to *observe* the sessions in
which doors get priced. Today the CHI plugin keeps one snapshot per capture and
throws the rest of the session away: every intermediate configuration, every
price recomputation as options are toggled, is lost.
...
* **Never record secrets.** Typed text and keystrokes are recorded as
  ...
  their Azure B2C password into this stream, and the corpus is rsynced to a
  laptop — so redaction happens at the source rather than in a later pass.
  ...
  **Known limitation, stated plainly:** `document.body.innerText` does NOT
  ...
  operator *types* is recorded only as a length. Values HubX renders as text —
  which on the door-summary page is the whole configuration (Model, Width,
  Height, Spring, Track, Cyclage: verified across all 87 production captures) —
  are captured in full. The gap is mid-configuration form state, and closing it
  needs a DOM snapshot that reads input values, not a relaxation of this
  redaction. Do not assume a complete input->price mapping until that lands.
...
# Query/fragment keys that carry a credential rather than a location. HubX's B2C
# sign-in uses response_mode=form_post so tokens normally arrive in a POST body,
# but that is a property of CHI's current config, not a guarantee — and a URL is
# recorded to disk and then rsynced to a laptop.
...
class SessionRecorder:
    ...
    def _note_client(self, ev: dict) -> None:
        ...
        if kind == "text":
            ...
            # password into HubX's B2C form, and the page-text snapshots already
