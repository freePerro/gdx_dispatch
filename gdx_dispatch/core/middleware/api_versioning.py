...
Added via ``app.add_middleware(APIVersioningMiddleware)`` in
``gdx_dispatch/app.py`` — after auth, before tracing, so
``request.state.api_version`` is populated when handlers run.
"""
